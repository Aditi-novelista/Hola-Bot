...
def main():
	...
	while not rospy.is_shutdown():
		...
		#print("Hola theta: " + str(hola_theta))
		...
		while abs(e_x) >= 2 or abs(e_y) >= 2 or abs(e_theta) >= 5:
			ct = hola_theta
			x0, y0 = hola_x, hola_y
			...
			res = np.matmul(mat, [vx, vy, omega])

			front_vel.force.x = -res[0]
			front_wheel_pub.publish(front_vel)
			right_vel.force.x = -res[1]
			right_wheel_pub.publish(right_vel)
			left_vel.force.x = -res[2]
			left_wheel_pub.publish(left_vel)
			print("vx: " + str(vx) + " vy: " + str(vy) + " w: " + str(omega))
			print("x: " + str(hola_x) + " y: " + str(hola_y) + " theta: " + str(hola_theta) + " ex: " + str(e_x) + " ey: " + str(e_y))
			print("v1: " + str(res[0]) + " v2: " + str(res[1]) + " v3: " + str(res[2]))
			rate.sleep()
			e_x = x - hola_x
			e_y = y - hola_y
			e_theta = theta_d - hola_theta
